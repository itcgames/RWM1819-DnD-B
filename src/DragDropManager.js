  ...
  checkDropZones() {
    if ((this.dropzones == undefined || this.dropzones.length <= 0) &&
      ...
      for (var j = 0; j < this.draggables.length; j++) { // first iterate through all the draggables
        ...
        if (this.draggables[j].origin.x != draggable.x || this.draggables[j].origin.y != draggable.y) {
          ...
          for (var i = 0; i < this.dropzones.length; i++) { // check each against all available dropzones
            ...
            if (utilities.boundingBoxCollision(draggable, dropzone) && this.dropzones[i].getCurrentCount() < this.dropzones[i].getCapacity()) { // if they are colliding
              var x, y;
              ...
              this.dropzones[i].setCount(this.dropzones[i].getCurrentCount() + 1);
              noCollision = false;
